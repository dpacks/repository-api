const dwid = require('@dwauth/core')
...
module.exports = function (config, db) {
  const dwidDb = level(config.dwid.db || path.join(__dirname, 'dwid.db'))
  const DWAUTH = dwid(dwidDb, config.dwid)
  const mx = Mixpanel.init(config.mixpanel || 'mixpanel disabled')
  var reset
  if (config.email) reset = createReset(config, dwidDb)
  ...
  function currentUser (req, cb) {
    ...
    DWAUTH.verify(req, function (err, decoded, token) {
  ...
  function register (req, res) {
    ...
    verify(req.body, {whitelist: config.whitelist}, function (err) {
      ...
      DWAUTH.register(req, res, {body: req.body}, function (err, statusCode, obj) {
        if (err) {
  ...
  function login (req, res) {
    ...
    DWAUTH.login(req, res, {body: body}, function (err, resp, obj) {
      if (err) {
  ...
  function passwordReset (req, res) {
    ...
    DWAUTH.accounts.findByEmail(userEmail, function (err, account) {
  ...
  function passwordResetConfirm (req, res) {
    ...
    DWAUTH.accounts.auth.update(options, function (err, account) {
