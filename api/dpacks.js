 ...
 * @param {Object} vaultr dat registry Vaultr instance
 */
...
/**
 * POST request for the Dat model. Used to create new dpacks. Must be logged in.
...
DPacks.prototype.post = function (req, res) {
  ...
  self._user(req, res, function (user) {
    ...
    if (!self.vaultr.opts.verifyConnection) return addDat(req, user)
    // let's only add dats if the url is up
    self.vaultr.get(req.body.url, function (err, archive, key) {
      if (err) return onerror(err, res)
      var timeout = req.body.timeout || 10000
      self.vaultr.metadata(archive, {timeout}, function (err, info) {
        if (err) return onerror(err, res)
        addDat(req, user)
      })
    })
...
/**
 * PUT request for the Dat model. Used to update dpacks. Must be logged in.
